...
import { generateImageWithBanana } from '../services/bananaService';
...
export const ImagePromptCard: React.FC = () => {
  ...
  // State for Banana.dev generation
  ...
  const handleGenerateRealImage = async (promptText: string) => {
    ...
    try {
      // Use the Banana service
      const imageUrl = await generateImageWithBanana(promptText, apiKey);
      
      // Update state with result and save API key for session
      updateState({ 
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {/* NEW: Banana.dev Real Image Generation Section */}
      {state.imagePrompts.length > 0 && (
        <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 shadow-inner">
          <h4 className="font-bold mb-4 text-gray-900 flex items-center gap-2">
            <ImageIcon size={18} /> Real Image Generation (Banana.dev)
          </h4>
          ...
          {!state.generatedImage ? (
            <div className="space-y-3">
              <input 
                type="password" 
                placeholder="Enter Banana API Key"
                className="w-full p-3 rounded-xl border border-gray-300 focus:border-primary outline-none text-sm"
                value={apiKey}
              ...
              >
                {isGeneratingImg ? "Rendering on GPU..." : "Generate Real Image"}
              </button>
            </div>
          ) : (
